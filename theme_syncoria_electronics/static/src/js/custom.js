odoo.define('theme_syncoria_electronics.custom_js', function (require) {
    "use strict";
    ...
    snippet_sellers.each(function () {
        ...
        var category_id = snippet_seller.data('active-categ') || 1;
    ...
    var snippet_dealweeks = $('.section-deal-blocks');
    snippet_dealweeks.each(function () {
        var snippet_dealweek = $(this);
        var params = snippet_dealweek.data();
        if (!snippet_dealweek.hasClass('syncoria-snippet-initialized')) {
            snippet_dealweek.addClass('syncoria-snippet-initialized');
            $.ajax({
                type: "get",
                url: "/syncoria/dealweeks",
                data: params,
                success: function (response) {
                    snippet_dealweek.html(response);
                    snippet_dealweek.find(".dealweek-slider").slick(dealweekSlider);
                },
                error: function (returnval) {
                    product_list.html('<div>Unable to connect with server.</div>');
                }
    ...
    var snippet_topfeatures = $('.section-top-features-blocks');
    snippet_topfeatures.each(function () {
        var snippet_topfeatures = $(this);
        var params = snippet_topfeatures.data();
        console.log(snippet_topfeatures);
        console.log(params);
        if (!snippet_topfeatures.hasClass('syncoria-snippet-initialized')) {
            snippet_topfeatures.addClass('syncoria-snippet-initialized');

            $.ajax({
                type: "get",
                url: "/syncoria/top_product_list",
                data: params,
                success: function (response) {
                    snippet_topfeatures.html(response);
                    snippet_topfeatures.find(".custom-slider").slick(topFeaturedSlider);
                },
                error: function (returnval) {
                    product_list.html('<div>Unable to connect with server.</div>');
                }
    ...
    var product_list_blocks = $('.section-feature-blocks');
    product_list_blocks.each(function () {
        var product_list = $(this);
        if (!product_list.hasClass('syncoria-snippet-initialized')) {
            product_list.addClass('syncoria-snippet-initialized');
            var params = product_list.data();
            $.ajax({
                type: "get",
                url: "/syncoria/product_lists",
                data: params,
                success: function (response) {
                    product_list.html(response);
                    product_list.find('#' + params.id + ' a').on('click', function (e) {
                        e.preventDefault();
                        $(this).tab('show');
                        if (!product_list.find(this.hash + " " + ".featured-slider").hasClass('slick-initialized'))
                            product_list.find(this.hash + " " + ".featured-slider").slick(featured_slider);
                    });
                    product_list.find('li:first-child a').click();
                },
                error: function (returnval) {
                    product_list.html('<div>Unable to connect with server.</div>');
                }
...
odoo.define('theme_syncoria_electronics.ProductConfiguratorMixin', function (require) {
    'use strict';
    require('web.dom_ready');
